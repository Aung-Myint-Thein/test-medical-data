...
## log
estimation_data[, "qutbill"] <- sqrt(estimation_data[, "HOSPITALBILL"])
estimation_data[, "qutbill"] <- log(estimation_data[, "qutbill"] + 1)
...
#estimation_data[, "logbillb"] <- apply(estimation_data, 1, function(row) ifelse(row["WARDTYPE"] == "", log(170), as.numeric(row["logbill"])))

for(i in 1:4){
  for(j in 1:4){
    lm4 <- lm(logbill ~ AGE + GENDER + DURATIONOFSTAY + YEAROFADM + AGE*GENDER + GENDER*DURATIONOFSTAY + ItemNo +
